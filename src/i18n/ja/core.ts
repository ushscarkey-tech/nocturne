...
const core: Partial<Record<keyof typeof source, string>> = {
  ...
  "change.returned": "{task}の残り{min}は{at}発の列車に乗り換え。",
  "change.returnedLater": "{task}の残り{min}は別の日の列車に乗り換え。",
  "change.rests": "{task}は今夜休み。",
  "change.deferred": "{task}（{min}）は別の日の列車に乗り換え。",
  "change.joins": "{task}（{min}）が{at}に乗車。",
