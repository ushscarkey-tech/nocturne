...
function LineDetail() {
  ...
  const unattached = data.tasks.filter(
    (t) =>
      t.lineId !== line.id && (t.status === "active" || t.status === "inbox"),
  );

  return (
    <div className="animate-fade">
      <BackLink />
      {/* Wide screens: the line at left, its tasks at right. */}
      <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,26rem)] lg:grid-rows-[auto_1fr] lg:gap-x-16 xl:grid-cols-[minmax(0,1fr)_minmax(0,29rem)] xl:gap-x-24">
        <div className="lg:col-start-1 lg:row-start-1">
          <header className="mt-8">
            <p className="eyebrow">
              {line.targetDate
                ? fmt.longDate(line.targetDate)
                : t("lines.noTargetDate")}
            </p>
          ...
          <dl className="mt-10 grid grid-cols-3 gap-6 border-t border-rule-soft pt-8">
            <div>
              ...
              <dd className="mt-1.5 font-mono text-2xl tabular text-lamp">
                {Math.round(p.fraction * 100)}%
              </dd>
            </div>
            <div>
              <dt className="eyebrow text-[0.625rem]">{t("lines.workload")}</dt>
              <dd className="mt-1.5 font-mono text-2xl tabular">
                {fmt.duration(p.total)}
              </dd>
            </div>
            <div>
        ...
        >
          ...
          {tasks.length === 0 && (
            <p className="mt-3 text-sm text-mist">{t("lines.attachEmpty")}</p>
          )}
        </section>

        <div className="mt-12 flex gap-3 border-t border-rule-soft pt-8 lg:col-start-1 lg:row-start-2 lg:self-start">
        ...
        {unattached.length === 0 ? (
          ...
          <ul className="divide-y divide-rule-soft">
            {unattached.map((task) => (
              ...
              >
                <span className="min-w-0 truncate text-sm text-paper-dim">
                  {task.title}
                </span>
                <Button
                  variant="quiet"
                  size="sm"
                  onClick={() => attachTask(task.id, line.id)}
                >
                  {t("lines.attach")}
                </Button>
              </li>
              ...
              {tasks.map((task) => (
                ...
                >
                  ...
                  <Button
                    variant="quiet"
                    size="sm"
                    onClick={() => attachTask(task.id, null)}
                  >
                    {t("lines.detach")}
                  </Button>
                </li>
      ...
      <Sheet
        open={sheet === "task"}
        onClose={() => setSheet(null)}
        title={t("lines.newTask")}
        eyebrow={line.title}
      >
        <TaskForm
          initial={draftFrom(undefined, {
            lineId: line.id,
            deadline: line.targetDate,
          })}
          lines={data.lines}
