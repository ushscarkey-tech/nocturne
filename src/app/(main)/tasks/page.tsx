...
function TasksView() {
  ...
  const tonight = useMemo(() => {
    ...
    for (const s of routeOf(data.sessions, today)) {
      if (s.status === "planned" || s.status === "active")
        m.set(s.taskId, (m.get(s.taskId) ?? 0) + s.plannedMinutes);
    }
    return m;
  }, [data.sessions, today]);
  ...
  const sections = useMemo(() => {
    ...
    for (const t of data.tasks) {
      ...
      if (t.status === "done") done.push(t);
        ...
        tonight.has(t.id) ||
        (t.deadline && t.deadline <= today && !t.recurrence) ||
        (t.recurrence && recursOn(t, today))
      )
        todayList.push(t);
  ...
  return (
    <div className="animate-fade">
      ...
      {/* Wide screens: what's on now at left, what's coming at right. */}
      <div className="lg:grid lg:grid-cols-2 lg:items-start lg:gap-x-16 xl:gap-x-20">
        <div>
          <TaskSection
            id="inbox"
            title={t("tasksPage.inbox")}
            hint={t("tasksPage.inboxHint")}
        ...
        <div>
          ...
          <TaskSection
            id="someday"
            title={t("tasksPage.someday")}
            hint={t("tasksPage.somedayHint")}
            ...
            <section className="mt-12">
              ...
              >
                ...
                <Icon
                  name="chevron"
                  size={12}
                  className={`transition-transform duration-500 ${showDone ? "rotate-90" : ""}`}
                />
              </button>
              {showDone && (
      ...
      <ul className="mt-2 divide-y divide-rule-soft">
        {tasks.map((t) => (
          <TaskRow
            ...
            task={t}
            today={today}
            tonightMinutes={tonight.get(t.id)}
          />
        ))}
      </ul>
    </section>
