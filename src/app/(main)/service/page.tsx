...
export default function ServicePage() {
  ...
  const exceptions = data.windows
    ...
    .sort((a, b) => `${a.specificDate}${a.startTime}`.localeCompare(`${b.specificDate}${b.startTime}`));
  const weekMinutes = Array.from({ length: 7 }, (_, i) => totalMinutes(availabilityForDate(data.windows, addDays(today, i)))).reduce((a, b) => a + b, 0);
  ...
  return (
    <div className="animate-fade">
      ...
      {editor && <WindowEditor editor={editor} onClose={() => setEditor(null)} />}
    </div>
  );
}
...
function WindowEditor({ editor, onClose }: { editor: Editor; onClose: () => void }) {
  ...
  const [start, setStart] = useState(w?.startTime ?? (editor.mode === "exception" && editor.kind === "blocked" ? "19:40" : "19:00"));
  ...
  function save() {
    ...
    if (isWeekly) {
      ...
      if (w) saveWindow({ ...w, dayOfWeek: days[0], startTime: start, endTime: end, enabled });
        ...
        saveWindows(
          days.map((day) => ({ dayOfWeek: day, specificDate: null, startTime: start, endTime: end, recurring: true, enabled: true, kind: "available" as const })),
        );
    } else {
      saveWindow({ id: w?.id, dayOfWeek: null, specificDate: date, startTime: start, endTime: end, recurring: false, enabled, kind });
    }
    onClose();
  }
    ...
    <Sheet open onClose={onClose} title={title} eyebrow="Service Time">
      <div className="space-y-7">
        {isWeekly ? (
          ...
          <Field label="Date">
            <input type="date" className="field font-mono" value={date} min={today} onChange={(e) => setDate(e.target.value)} />
          </Field>
        )}
        <div className="grid grid-cols-2 gap-6">
          <Field label="From">
            <input type="time" className="field font-mono text-lg" value={start} onChange={(e) => setStart(e.target.value)} step={300} />
          </Field>
          <Field label="Until">
