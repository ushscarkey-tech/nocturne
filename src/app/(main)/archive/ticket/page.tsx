...
function TicketDetail() {
  ...
  const titleOf = (taskId: string) =>
    data.tasks.find((t) => t.id === taskId)?.title ?? t("archive.removedTask");
  ...
  const details = (
    <>
      <dl className="grid grid-cols-2 gap-x-8 gap-y-6 sm:grid-cols-4">
        <Stat
          label={t("archive.statPlanned")}
          value={fmt.duration(s.plannedMinutes)}
        />
        <Stat
          label={t("archive.statFocused")}
          value={fmt.duration(s.focusedMinutes)}
        />
        <Stat
          label={t("archive.statStations")}
          value={`${s.stationsCompleted} / ${s.stationsTotal}`}
        />
        <Stat
          label={t("archive.statRouteChanges")}
          value={String(journey.routeChanges)}
        />
        <Stat
          label={t("archive.statDelay")}
          value={delayed ? fmt.duration(delayed) : "—"}
        />
        <Stat
          label={t("archive.statAheadOfSchedule")}
          value={gained ? fmt.duration(gained) : "—"}
        />
        <Stat
          label={t("archive.statCarriage")}
          value={t(
            (CARRIAGES.find((c) => c.id === journey.selectedCarriage)
              ?.nameKey ?? "journey.quietCar") as MessageKey,
          )}
        />
        <Stat
          label={t("archive.statSeat")}
          value={t("archive.carSeat", { car: journey.car, seat: journey.seat })}
        />
      </dl>

      <section
        className="mt-10 border-t border-rule-soft pt-8"
          ...
          {s.stations.map((st) => (
            ...
            >
              <span className="min-w-0">
                <span className="block truncate text-paper-dim">
                  {titleOf(st.taskId)}
                </span>
                <span className="eyebrow text-[0.6rem] text-haze">
                  {fmt.station(st.stationName)} ·{" "}
                  {clock(st.actualStart ?? st.plannedStart)}
                </span>
              </span>
              <span className="shrink-0 text-right font-mono text-xs tabular text-mist">
                {st.status === "done" || st.status === "partial"
                  ? fmt.duration(st.completedMinutes)
                  : t("archive.notReached")}
                {st.status === "partial" && (
                ...
                {st.status === "done" &&
                  st.completedMinutes !== st.workMinutes && (
                    <span className="block text-haze">
          ...
          <ul className="mt-3 space-y-4">
            {journey.changeLog.map((c) => (
              <li key={c.at} className="border-l border-rule pl-4 text-sm">
                <p className="font-mono text-xs text-haze tabular">
                  {clock(c.at)}
                </p>
  ...
  return (
    <div className="animate-fade">
      ...
      <BackLink />
      {/* Wide screens: the ticket at left, the journey it records at right. */}
      <div className="lg:mt-4 lg:grid lg:grid-cols-[auto_minmax(0,1fr)] lg:items-start lg:gap-x-20 xl:gap-x-28">
        <div className="lg:sticky lg:top-16">
          <header className="mt-6 text-center">
            <p className="eyebrow">
              {issued ? t("archive.ticketIssued") : t("archive.journey")}
            </p>
            <h1 className="mt-1.5 font-display text-3xl leading-tight">
              {fmt.longDate(journey.date)}
            </h1>
          </header>
          ...
          <div className="nocturne-unfold mt-6 flex justify-center">
            <Ticket
              face={face}
              style={ticket?.ticketStyle ?? journey.selectedCarriage}
              size="lg"
            />
          </div>
        ...
        <section
          ...
          aria-label={t("archive.journeyDetails")}
        >
          <h2 className="eyebrow mb-8">{t("archive.journeyDetails")}</h2>
          {details}
        </section>
      </div>
      ...
      <Sheet
        ...
        title={t("archive.journeyDetails")}
        eyebrow={fmt.longDate(journey.date)}
        wide
      >
        {details}
      </Sheet>
