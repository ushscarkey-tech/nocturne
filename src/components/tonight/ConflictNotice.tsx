...
export function ConflictNotice({
  ...
  return (
    <>
      <RescuePlan data={data} now={now} />
      <section
        aria-labelledby="conflict-title"
        className="mt-6 animate-rise border-l border-signal/60 py-1 pl-5"
      >
        ...
        <p className="mt-2 max-w-md text-sm leading-relaxed text-paper-dim">
          {t("conflict.notEnoughTime", {
            date: fmt.shortDate(conflict.deadline),
          })}
        </p>
        <ConflictFigures conflict={conflict} />
        <div className="mt-5 flex flex-wrap items-center gap-3">
          <Button variant="secondary" size="sm" onClick={() => setOpen(true)}>
            {t("conflict.orYourself")}
          </Button>
    ...
    <dl className="mt-4 grid max-w-sm grid-cols-3 gap-4 font-mono tabular">
      <div>
        ...
        <dd className="mt-1 text-paper">
          {fmt.duration(conflict.requiredMinutes)}
        </dd>
      </div>
      <div>
        <dt className="eyebrow text-[0.625rem]">{t("conflict.available")}</dt>
        <dd className="mt-1 text-paper">
          {fmt.duration(conflict.availableMinutes)}
        </dd>
      </div>
      <div>
        <dt className="eyebrow text-[0.625rem]">{t("conflict.shortfall")}</dt>
        <dd className="mt-1 text-signal">
          {fmt.duration(conflict.shortfallMinutes)}
        </dd>
      </div>
    </dl>
  );
...
function ResolveSheet({
  ...
  return (
    <Sheet
      open={open}
      onClose={onClose}
      title={conflict ? t("conflict.resolveTitle") : t("conflict.clearTitle")}
              ...
              <li key={task.id} className="py-4">
                <div className="flex items-baseline justify-between gap-3">
                  <Link
                    ...
                    className="min-w-0 truncate text-paper hover:underline"
                  >
                    {task.title}
                  </Link>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        updateTask(task.id, {
                          ...
                          remainingMinutes: task.remainingMinutes - 30,
                        })
                      }
                    >
