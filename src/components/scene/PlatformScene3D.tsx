...
export default function PlatformScene3D({ mood = "waiting", rain = true, stationName = "NOCTURNE", className = "" }: PlatformSceneProps) {
  ...
  useEffect(() => {
    ...
    // --------------------------------------------------------------- environment
    // A tiny studio for metal to reflect: a dark sky and one warm strip overhead.
    ...
    const envMap = track(pmrem.fromScene(envScene, 0.03).texture);
    ...
    const mat = {
      ...
      rail: track(new THREE.MeshStandardMaterial({ color: 0x9a9d9b, metalness: 1, roughness: 0.3, envMap })),
    ...
    const bloom = new UnrealBloomPass(new THREE.Vector2(256, 256), 0.36, 0.3, 1.0);
